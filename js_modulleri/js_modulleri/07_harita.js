// Ne işe yarar: SVG harita etkileşimlerini, toprak sahipliklerini, savaş sisini ve stratejik bölgeleri yönetir.
...
function getTurkishMapName(value){
 ...
 return raw.replace(/\bProvince\b/gi,'').replace(/\bCounty\b/gi,'').replace(/\bRegion\b/gi,'').replace(/\s{2,}/g,' ').trim();
}
...
function getStrategicRegionList(){
 ...
 Object.values(groups).forEach(group=>{(group.paths||[]).forEach(provinceId=>{if(!seen.has(provinceId)){seen.add(provinceId);out.push({id:provinceId,name:getTurkishMapName(provinceId)});}});});
 if(!out.length){Object.keys(db.mapProvinceOwners||{}).forEach(provinceId=>{if(!seen.has(provinceId)){seen.add(provinceId);out.push({id:provinceId,name:getTurkishMapName(provinceId)});}});}
...
function renderStrategicRegionStickers(){
 ...
 Object.entries(db.valuableRegions||{}).forEach(([provinceId,cfg])=>{
   ...
   try{
     const b=path.getBBox(); const area=Math.max(1,b.width*b.height);
 ...
 const rows=regions.map((r,i)=>{
   ...
   return `<div class="list-item strategic-region-row" data-strategic-region-row data-search="${esc(r.name)}" style="display:block;margin-bottom:7px;padding:8px;">
     ...
     <div class="formgrid" style="margin:0;">
       ...
       ${field(`sr_scale_${k}`,'Sticker Ölçeği',cfg.scale||1,'number')}
     </div>
   </div>`;
 }).join('');
...
</div><div style="max-height:58vh;overflow:auto;padding-right:4px;">${rows||'<p class="sub">Harita bölgeleri yüklenemedi.</p>'}</div>
 <div class="actions" style="margin-top:12px;"><button class="btn" onclick="openAdmin()">GERİ</button><button class="btn blue" onclick="saveStrategicRegions()">💾 KAYDET</button></div>`);
}
function saveStrategicRegions(){
 ...
 db.valuableRegions=next; queueMapSave(); closeModal();
...
function applyMapOwnership(){
 ...
 if(isAdmin) {
     ...
     if(simId) { viewer = getState(simId); isSimulating = true; } 
 }
 ...
 if((!isAdmin || isSimulating) && viewer && (viewer.istihbarat_binasi || 0) === 0) {
    ...
    if(myProvinces.length > 0) {
        ...
        myProvinces.forEach(id => { (mapAdjacencyCache[id] || []).forEach(adjId => visibleProvinces.add(adjId)); });
    }
 }
...
async function openGameMap(){
 ...
 try{
   ...
   mapZoom=5;changeMapZoom(0);applyMapOwnership();
   const mapViewport=document.getElementById('gameMapViewport');if(mapViewport){mapViewport.scrollLeft=Math.max(0,(mapViewport.scrollWidth-mapViewport.clientWidth)*.62);mapViewport.scrollTop=Math.max(0,(mapViewport.scrollHeight-mapViewport.clientHeight)*.15);}
...
function updateMapBatchUi(){const btn=document.getElementById('mapBatchBtn'),status=document.getElementById('mapBrushStatus');if(btn){btn.textContent=mapBatch?`✓ ATAMAYI BİTİR (${mapBatchSelection.size})`:'☑ ÇOKLU ATAMA';btn.className=mapBatch?'btn gold':'btn green';}if(status&&mapBatch)status.textContent=`Çoklu seçim: ${mapBatch.countryName} • ${mapBatchSelection.size} toprak seçildi.`;}
...
function pickProvinceAsBrush(provinceId){
 ...
 const brushBtn=document.getElementById('mapBrushBtn');if(brushBtn){brushBtn.textContent='🖌️ FIRÇA';brushBtn.className='btn gold';}
 updateMapBatchUi();
 const status=document.getElementById('mapBrushStatus');if(status)status.textContent=`${countryName} seçildi. Şimdi atanacak bütün topraklara dokun.`;
